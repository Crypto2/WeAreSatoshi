...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, std::string& strFailReason, std::string strTxComment)
{
    int64 nValue = 0;
    ...
    // transaction comment
    wtxNew.nVersion = CurrentTransactionVersion();
    if (wtxNew.nVersion > CTransaction::ORIGINAL_VERSION)
    {
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, std::string& strFailReason, std::string strTxComment)
{

...
static const int64 nMinSubsidy = 1000 * COIN;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        /* Don't allow old clients now that the comment hardfork is active */
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WeAreSatoshiMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WeAreSatoshiMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("WeAreSatoshiMiner started\n");
    ...
    try { loop {
        ...
        printf("Running WeAreSatoshiMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WeAreSatoshiMiner terminated\n");
        throw;
    }
}
